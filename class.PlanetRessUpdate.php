...
//grep -r "../X-Wing/" -e "Navigate carefully"
...
class ResourceUpdate
{
	...
	public function CalcResource($USER = NULL, $PLANET = NULL, $SAVE = false, $TIME = NULL)
	{
		...
		$this->TIME		= date('U'); //is_null($TIME) ? TIMESTAMP : $TIME;
                $this->USER['b_tech']=0;
	      /*  echo '<pre>';
                print_r($this->USER);
                echo '</pre><pre>';
                print_r($this->PLANET);
                echo '</pre>';
                echo 'PLANET=$GLOBALS[\''.$GLOBALS['PLANET'].'\']<br>';		*/
	...
	private function UpdateRessource()
	{
		...
		echo $resource[113].'<br>';
		$this->PLANET['metal_max']			= floor(2.5 * pow(1.8331954764, $this->PLANET['metal_store'])) * 5000  * $CONF['resource_multiplier'] * STORAGE_FACTOR;
		$this->PLANET['crystal_max']		= floor(2.5 * pow(1.8331954764, $this->PLANET['crystal_store'])) * 5000  * $CONF['resource_multiplier'] * STORAGE_FACTOR;
		$this->PLANET['deuterium_max']		= floor(2.5 * pow(1.8331954764, $this->PLANET['deuterium_store'])) * 5000  * $CONF['resource_multiplier'] * STORAGE_FACTOR;
		$this->PLANET['norio_max']		    = 1;
		...
		else
		{
			...
			$Caps['metal_perhour'] = $Caps['crystal_perhour'] = $Caps['deuterium_perhour'] = $Caps['norio_perhour'] = $Caps['energy_used'] = $Caps['energy_max'] = $Caps['deuterium_used'] = 0;
                 echo '<br><per>'; print_r($reslist['prod']); echo '<br></per>';
